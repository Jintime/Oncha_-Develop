package com.oncha.oncha_web.config.handler;
...
public class PathRequestMappingHandlerMapping extends RequestMappingHandlerMapping {
    ...
    protected RequestMappingInfo getMappingForMethod(Method method, Class<?> handlerType) {
        ...
        if (!superclassUrlPatterns.isEmpty()) {
            RequestMappingInfo superclassRequestMappingInfo = RequestMappingInfo.paths(String.join("", superclassUrlPatterns)).options(getBuilderConfiguration()).build();
            RequestMappingInfo result =  superclassRequestMappingInfo.combine(requestMappingInfo);
            return result;
        } else {
